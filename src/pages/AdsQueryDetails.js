...
const AdsQueryDetails = (props) => {
  ...
  useEffect(() => {
    const Livechaturl = `http://3.23.210.57:3000/api/v1/auth/detailsadsquery`;
    axios.post(Livechaturl, { id: params.queryId }, { 'Content-Type': 'multipart/form-data' })
      .then((response) => {
    ...
    const LoginSchema = Yup.object().shape({
        reply_message: Yup.string().required('reply_message is required'),
    ...
    const formik = useFormik({
        ...
        onSubmit: () => {
            ...
            // bodyFormData.append('id', params.reportId);
            // bodyFormData.append('reply_message', values.reply_message);

            const Livechaturl = `http://3.23.210.57:3000/api/v1/auth/updateadsquery`;
            axios.post(Livechaturl, {
                id: params.queryId,
                reply_message: values.reply_message
              }).then((response) => {
                    const outlook = response
                    navigate('/dashboard/adsquery', { replace: true });
  ...
  return (
    <Page title="Ads Query Details | SoundChatRadio">
                    ...
                    <Card>
                      ...
                      <CardContent>
                        <ul className="list-group-left">
                          <li className="list-group-item list-group-item-primary">Query Id</li>
                          ...
                          <li className="list-group-item list-group-item-primary">Mobile</li>
                          <li className="list-group-item list-group-item-primary">Query Status</li>
                          <li className="list-group-item list-group-item-primary">Query Subject</li>
                          <li className="list-group-item list-group-item-primary">Query Date</li>
                        </ul>
                        <ul className="list-group-right">
                          <li className="list-group-item list-group-item-primary">{gallery.query_id}</li>
                          <li className="list-group-item list-group-item-primary">{gallery.user_name}</li>
                          <li className="list-group-item list-group-item-primary">{gallery.email}</li>
                          <li className="list-group-item list-group-item-primary">{gallery.mobile}</li>
                          <li className="list-group-item list-group-item-primary">{gallery.query_status}</li>
                          <li className="list-group-item list-group-item-primary">{gallery.subject}</li>
                  ...
                  <Grid item xs={12} md={12} lg={12}>
                                                <Stack spacing={3}>
                                                    <FormControl>
                                                        <TextField
                                                            fullWidth
                                                            autoComplete="reply_message"
                                                            type="text"
                                                            label="Reply Content"
                                                            multiline
                                                            rows={5}
                                                            rowsMax={10}
                                                            {...getFieldProps('reply_message')}
                                                            error={Boolean(touched.reply_message && errors.reply_message)}
                                                            helperText={touched.reply_message && errors.reply_message}
                                                        />
                                                    </FormControl>
                                                </Stack>
...
export default AdsQueryDetails;