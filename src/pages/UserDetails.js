...
const UserDetails = (props) => {
  ...
  useEffect(() => {
    const Livechaturl = `http://3.23.210.57:3000/api/v1/auth/getprofile`;
    axios.post(Livechaturl, { user_id: params.userId }, { 'Content-Type': 'multipart/form-data' })
      .then((response) => {
        const outlook = response
        setgallery(outlook.data.data);
        // window.location.href = '/dashboard/gallery';
      })
      const profileurl = `http://3.23.210.57:3000/api/v1/auth/subscriberdetails`;
    axios.post(profileurl, { user_id: params.userId }, { 'Content-Type': 'multipart/form-data' })
      .then((response) => {
        const outlook = response
                    ...
                    <Card>
                      ...
                      <CardContent>
                        ...
                        <ul className="list-group-right">
                            <li>
                                {/* <img alt={gallery.fname} src={baseurl + gallery.profilpic}/> */}
                            <Avatar alt={gallery.fname} src={baseurl + gallery.profilpic} /></li>
                          <li className="list-group-item list-group-item-primary">{gallery.username}</li>
                          <li className="list-group-item list-group-item-primary">{gallery.fname}</li>
