    ...
    return (
        <form className={classes.root} onSubmit={handleSubmit(onSubmit)}>
            <Stack direction={{ xs: 'row', sm: 'row', md: 'row', lg: 'row' }} spacing={2}>
                {/* <img src={files1?.source || defaultSrc} alt="preview" width='200' height='200'/>
      <Button
        onClick={() =>
          selectFiles1({ accept: "image/*" }, ({ name, size, source, file }) => {
      ...
      >
        Upload Banner For Mobile
            ...
            <Stack direction={{ xs: 'row', sm: 'row', md: 'row', lg: 'row' }} spacing={2}>
                {/* <img src={files2?.source || defaultSrc} alt="preview" width='200' height='200'/>
      <Button
        onClick={() =>
          selectFiles2({ accept: "image/*" }, ({ name, size, source, file }) => {
            console.log("Files Selected", { name, size, source, file });
          })
        }
      >
        Upload Banner For Desktop
      </Button> */}
