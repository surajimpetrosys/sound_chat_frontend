...
const AddFormGallery = () => {
 ...
 const fileSelectedHandler = (e) => {
  ...
  const onSubmit = data => {
    ...
    bodyFormData.append('content_type', data.content_type);
    bodyFormData.append('img_gallery_pic', state);
    bodyFormData.append('gallery_pic', state);

    const Livechaturl = `http://3.23.210.57:3000/api/v1/auth/addgallery`;
  ...
  return (
    <form className={classes.root} onSubmit={handleSubmit(onSubmit)}>
      ...
      />
       <Controller
        name="img_gallery_pic"
        control={control}
        defaultValue=""
        render={({ field: { onChange, value }, fieldState: { error } }) => (
          <TextField
            label="gallery pics Array"
            variant="filled"
            value={value}
            onChange={onChange}
            error={!!error}
