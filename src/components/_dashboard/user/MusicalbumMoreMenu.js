...
export default function MusicalbumMoreMenu(props) {
  ...
  const deleterow = ()=>{
    const Livechaturl = `http://3.23.210.57:3000/api/v1/auth/deletemusicalbum`;
        axios.post(Livechaturl, {
          id: props.id,
        }).then((response) => {
          const outlook = response
          window.location.href = '/dashboard/music-album';
  ...
  return (
    <>
      ...
      >
        <MenuItem component={RouterNavLink} to={`/dashboard/music-album/updatemusicalbum/${props.id}`}  sx={{ color: 'text.secondary' }}>
          <ListItemIcon>
            <Icon icon={editFill} width={24} height={24} onClick={handleClickOpen} />
          </ListItemIcon>
        </MenuItem>
        <MenuItem component={RouterNavLink} to={`/dashboard/music-album/details/${props.id}`}  sx={{ color: 'text.secondary' }}>
          <ListItemIcon>
