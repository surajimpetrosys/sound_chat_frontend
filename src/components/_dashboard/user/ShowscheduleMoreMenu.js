...
export default function ShowscheduleMoreMenu(props) {
  console.log(props)
  ...
  const deleterow = ()=>{
    const Livechaturl = `http://3.23.210.57:3000/api/v1/auth/deleteperdayschedule`;
        axios.post(Livechaturl, {
          day_id: props.dayid,
          show_id: props.showid
        }).then((response) => {
          const outlook = response
          window.location.href = `/dashboard/shows/showslist/${props.dayid}`;
  ...
  const editrow = ()=>{
    alert(JSON.stringify(props.dayid))
  }
  return (
    <>
      ...
      >
        <MenuItem component={RouterNavLink} to={`/dashboard/shows/showslist/updateshowschedule/${props.dayid}/${props.showid}`}  sx={{ color: 'text.secondary' }}>
          <ListItemIcon>
