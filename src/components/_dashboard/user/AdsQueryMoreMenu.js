...
export default function AdsQueryMoreMenu(props) {
  ...
  const deleterow = ()=>{
    const Livechaturl = `http://3.23.210.57:3000/api/v1/auth/deleteadsquery`;
        axios.post(Livechaturl, {
          id: props.id,
        }).then((response) => {
          const outlook = response
          window.location.href = '/dashboard/adsquery';
  ...
  return (
    <>
      ...
      >
        ...
        <MenuItem component={RouterNavLink} to={`/dashboard/adsquery/details/${props.id}`}  sx={{ color: 'text.secondary' }}>
          <ListItemIcon>
