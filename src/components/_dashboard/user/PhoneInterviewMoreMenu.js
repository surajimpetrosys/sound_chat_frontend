...
export default function PhoneInterviewMoreMenu(props) {
  ...
  const deleterow = ()=>{
    const Livechaturl = `http://3.23.210.57:3000/api/v1/auth/deletePhoneinterviews`;
        axios.post(Livechaturl, {
          id: props.id,
        }).then((response) => {
          const outlook = response
          window.location.href = '/dashboard/phoneinterview';
  ...
  return (
    <>
      ...
      >
        <MenuItem component={RouterNavLink} to={`/dashboard/phoneinterview/updateinterview/${props.id}`}  sx={{ color: 'text.secondary' }}>
          <ListItemIcon>
        ...
        <MenuItem component={RouterNavLink} to={`/dashboard/phoneinterview/details/${props.id}`}  sx={{ color: 'text.secondary' }}>
          <ListItemIcon>
